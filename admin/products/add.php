...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $chk = $conn->query("SELECT id FROM products WHERE slug = '$slugEsc'");
    ...
    $img1Result = uploadProductImage('image',  $seoBase,          $uploadDir);
    $img2Result = uploadProductImage('image2', $seoBase . '-2',   $uploadDir);
    $img3Result = uploadProductImage('image3', $seoBase . '-3',   $uploadDir);
    $img4Result = uploadProductImage('image4', $seoBase . '-4',   $uploadDir);

    foreach ([
        [$img1Result, &$image,  'Main product image'],
        [$img2Result, &$image2, 'Image 2'],
        [$img3Result, &$image3, 'Image 3'],
        [$img4Result, &$image4, 'Image 4'],
    ] as [$result, &$var, $label]) {
        if (is_array($result)) { $errors[] = $result['error']; }
        elseif ($result)       { $var = $result; }
    }
    if (!$image) $image = 'default.jpg';
    $og_image = $image !== 'default.jpg' ? $image : '';
    ...
    if (empty($errors)) {
        ...
        $stmt->bind_param(
            ...
            // TOTAL types = 5+3+6+2+4+1i+3+4+1d+3i+4+4+3+3 = 46
            "sssss" . "sss" . "ssssss" . "ss" . "ssss" . "isss" . "ssss" . "dsii" . "ssss" . "ssss" . "sss" . "sss",
            $name, $slug, $category, $description, $full_description,
            $specifications, $features, $applications,
            $brand, $sku, $weight, $dimensions, $material, $color,
            $warranty, $certifications,
...
<style>
    ...
    .img-upload-zone.main-zone { padding: 2rem 1rem; }
    .img-upload-zone.main-zone .upload-icon { font-size: 2.5rem; }
    ...
    /* Dynamic row */
    .dynamic-row { background:var(--yellow-light); border:1px solid var(--yellow-border); border-radius:0.5rem; padding:0.6rem 0.75rem; margin-bottom:0.5rem; }
        ...
        <div class="content container-fluid pt-4 pb-5">
            ...
            <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center mb-4">
                <div class="d-flex align-items-center gap-3">
                    ...
                    <div>
                        <h3 class="fw-bolder text-dark mb-1">Add Product</h3>
                ...
                <div class="row g-4">
                                ...
                                <div class="row g-3">
                                    <div class="col-md-8">
                                        ...
                                        <input type="text" name="name" id="productName" class="form-control"
                                            placeholder="e.g. Heavy Duty Steel Bars" value="<?= $p('name') ?>" required>
                                        <div class="char-counter"><span>Name</span><span class="count" id="nameCount">0 chars</span></div>
                                    </div>
                                    <div class="col-md-4">
                                        <label class="form-label">Category <span class="text-danger">*</span></label>
                                        <input type="text" name="category" class="form-control"
                                            placeholder="Construction, Electrical..." value="<?= $p('category') ?>" required>
                                    </div>
                                    <div class="col-md-8">
                                        <label class="form-label">URL Slug</label>
                                        <div class="input-group">
                                            <input type="text" name="slug" id="productSlug" class="form-control"
                                                placeholder="auto-from-product-name" value="<?= $p('slug') ?>">
                                            <button type="button" class="btn btn-light border" id="generateSlug" title="Generate from name">
                                                <i class="fa fa-sync-alt"></i>
                                            </button>
                                        </div>
                                        <small class="text-muted" style="font-size:0.72rem;">Lowercase, hyphens only. e.g. heavy-duty-steel-bars</small>
                                    </div>
                                    <div class="col-md-4">
                                    ...
                                    <div class="col-12">
                                        ...
                                        <textarea name="description" id="shortDesc" class="form-control" rows="2"
                                            placeholder="One or two lines describing the product for catalogue listings..."><?= $p('description') ?></textarea>
                                        <div class="char-counter"><span>Ideal: 80–200 chars</span><span class="count" id="shortDescCount">0 chars</span></div>
                                    </div>
                                    <div class="col-12">
                                        <label class="form-label">Full Description</label>
                                        <textarea name="full_description" class="form-control" rows="6"
                                            placeholder="Detailed product description. Include use cases, advantages, technical highlights..."><?= $p('full_description') ?></textarea>
                                    </div>
                                </div>
                            </div>
                        </div>
                                ...
                                <div class="row g-3">
                                    ...
                                    <div class="col-md-6">
                                        ...
                                        <textarea name="features" class="form-control" rows="5"
                                            placeholder="High tensile strength&#10;Corrosion resistant&#10;BIS certified&#10;Easy to cut and bend"><?= $p('features') ?></textarea>
                                    </div>
                                    <div class="col-md-6">
                                        ...
                                        <textarea name="applications" class="form-control" rows="5"
                                            placeholder="Residential buildings&#10;Bridges and flyovers&#10;Foundation reinforcement&#10;Commercial complexes"><?= $p('applications') ?></textarea>
                                    </div>
                                    <div class="col-md-6">
                                    ...
                                    <div class="col-12">
                                        ...
                                        <input type="text" name="tags" class="form-control"
                                            placeholder="steel bars, tmt, construction, nagpur, fe500d..."
                                            value="<?= $p('tags') ?>">
                                    </div>
                                </div>
                            </div>
                                    ...
                                    <div class="col-md-4">
                                        ...
                                        <div class="form-check form-switch mt-2">
                                            <input class="form-check-input" type="checkbox" name="in_stock" id="inStock"
                                                ...
                                                <?= !isset($_POST['in_stock']) || !empty($_POST['in_stock']) ? 'checked' : '' ?>>
                            ...
                            <div class="card-body p-4">
                                ...
                                <div class="tab-content">
                                    ...
                                    <div class="tab-pane fade" id="tab-og">
                                        <div class="alert alert-warning border-0 rounded-3 p-3 mb-4 d-flex gap-2 align-items-center" style="background:var(--yellow-subtle); color:var(--yellow-dark); border-color:var(--yellow-border);">
                                    ...
                                    <div class="tab-pane fade" id="tab-twitter">
                                        <div class="alert border-0 rounded-3 p-3 mb-4 d-flex gap-2 align-items-center" style="background:var(--yellow-subtle); color:var(--yellow-dark);">
                    ...
                    <div class="col-xl-4 col-lg-5">
                            ...
                            <div class="card-body p-4">
                                <div class="d-flex align-items-center justify-content-between mb-4">
                                    ...
                                    <div class="form-check form-switch mb-0">
                                        <input class="form-check-input publish-toggle" type="checkbox" name="is_active" id="isActive"
                        ...
                        <div class="card border-0 shadow-sm rounded-4 mb-4">
                            ...
                            <div class="card-body p-4">
                                <div class="mb-3">
                                    ...
                                    <input type="text" name="badge" class="form-control" placeholder="Bestseller, New, Sale..." value="<?= $p('badge') ?>">
                                </div>
                                <div>
                                    <label class="form-label">Badge Type</label>
                                    <div class="d-flex flex-wrap gap-2 mt-1" id="badgeTypeGroup">
                                        <?php foreach (['bestseller'=>'bg-warning text-dark','new'=>'bg-success text-white','sale'=>'bg-danger text-white','featured'=>'bg-primary text-white','hot'=>'bg-orange text-white'] as $bt => $cls): ?>
                                        <button type="button"
                                            class="btn btn-sm badge-type-btn rounded-pill fw-semibold <?= $cls ?>"
                                            style="opacity:0.5; font-size:0.72rem;"
                                            data-val="<?= $bt ?>"
                        ...
                        <div class="card border-0 shadow-sm rounded-4 mb-4">
                            ...
                            <div class="card-body p-4">
                                <!-- Main -->
                                <label class="form-label mb-2">Main Image <span class="text-danger">*</span></label>
                                ...
                                <div class="row g-2">
                                    ...
                                    <div class="col-4">
                                        <div class="img-upload-zone" id="zone_image<?= $n ?>" onclick="document.getElementById('input_image<?= $n ?>').click()">
                                            <div class="upload-icon" style="font-size:1.2rem;"><i class="fa fa-plus"></i></div>
                                            <p style="font-size:0.7rem;">Image <?= $n ?></p>
                                ...
                                <small class="text-muted mt-2 d-block text-center" style="font-size:0.7rem;">All images auto-converted to WebP for performance</small>
                            </div>
                        </div>
                        ...
                        <div class="card border-0 shadow-sm rounded-4 mb-4">
                            <div class="card-header bg-white border-bottom py-3 px-4 card-section-header">
                                <h6 class="mb-0 fw-bold text-dark text-uppercase small" style="letter-spacing:0.5px;">
                                    <i class="fa fa-lightbulb me-2" style="color:var(--yellow-primary);"></i>SEO Tips for Products
                                </h6>
                            </div>
                            <div class="card-body p-4">
                                <ul class="list-unstyled mb-0 small text-muted" style="line-height:2.2;">
                                    <li>✅ Include product name + brand in meta title</li>
                                    <li>✅ Mention city in focus keyword (e.g. "nagpur")</li>
                                    <li>✅ Add certifications — boosts E-E-A-T signals</li>
                                    <li>✅ Use <strong>Product</strong> schema for Google Shopping</li>
                                    <li>✅ Short, clean slug: <code>heavy-duty-steel-bars</code></li>
                                    <li>✅ Upload multiple images for rich results</li>
                                    <li>✅ Fill specifications — improves content depth</li>
                                </ul>
                            </div>
                        </div>
...
document.getElementById('productName').addEventListener('input', function () {
    ...
    if (!document.getElementById('productSlug').dataset.edited) {
        document.getElementById('productSlug').value = autoSlug(this.value);
        document.getElementById('serpSlug').textContent = autoSlug(this.value) || 'product-slug';
    }
});
document.getElementById('generateSlug').addEventListener('click', function () {
    const slug = autoSlug(document.getElementById('productName').value);
    document.getElementById('productSlug').value = slug;
    document.getElementById('productSlug').dataset.edited = '';
    document.getElementById('serpSlug').textContent = slug || 'product-slug';
});
document.getElementById('productSlug').addEventListener('input', function () {
    this.dataset.edited = '1';
    document.getElementById('serpSlug').textContent = this.value || 'product-slug';
});
...
function setBadgeType(val, btn) {
    ...
    document.getElementById('productForm').querySelector('[name="badge"]').value = val.charAt(0).toUpperCase() + val.slice(1);
}
// Init active badge
(function(){
    const cur = document.getElementById('badgeTypeInput').value;
    if (cur) {
        const btn = document.querySelector(`.badge-type-btn[data-val="${cur}"]`);
